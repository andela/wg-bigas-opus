...
def add(request):
    ...
    return HttpResponseRedirect(reverse('nutrition:plan:view',
                                        kwargs={'id': plan.id}))
    ...
    * http://www.reportlab.com/apis/reportlab/dev/platypus.html
    ...
    for meal in plan.meal_set.select_related():
        ...
        for item in meal.mealitem_set.select_related():
            ...
            data.append([Paragraph(u"{0}{1}".format(item.amount, unit_name),
                        styleSheet["Normal"]),
                         p])
    ...
    # There is nothing to output
        ...
        t = Paragraph(_('<i>This is an empty plan, what \
                        did you expect on the PDF?</i>'),
                      styleSheet["Normal"])
    ...
    data = []
    data.append([Paragraph(u'<para align="center">{0}</para>'
                .format(_('Nutritional data')),
                 styleSheet["Bold"])])
    ...
    data.append([Paragraph(_('Fat'), styleSheet["Normal"]),
                 ...
                 Paragraph(six.text_type(plan_data['per_kg']['fat']),
                           styleSheet["Normal"])])
    data.append([Paragraph(_('Saturated fat content in fats'),
                styleSheet["Normal"]),
