...
class NutritionPlan(models.Model):
    ...
    description = models.TextField(max_length=2000,
                                   ...
                                   help_text=_('A description of the goal \
                                                of the plan, e.g. '
                                               '"Gain mass" or "Prepare \
                                                 for summer"'))
    has_goal_calories = models.BooleanField(verbose_name=_('Use daily \
                                                           calories'),
                                            default=False,
                                            help_text=_("Tick the box if you \
                                                         want to mark this "
                                                        "plan as having a goal \
                                                         amount of calories. "
                                                        "You can use the \
                                                         calculator or \
                                                         enter the "
    ...
    def get_closest_weight_entry(self):
        ...
        if abs(closest_entry_gte.date - target) < abs(closest_entry_lte.date -
                                                      target):
    ...
    def get_calories_approximation(self):
        ...
        # Within 3%
        if (actual_calories < goal_calories * 1.03) and (actual_calories >
                                                         goal_calories * 0.97):
            return 1
        # within 7%
        elif (actual_calories < goal_calories * 1.07) and (actual_calories >
                                                           goal_calories *
                                                           0.93):
            return 2
        # within 10%
        elif (actual_calories < goal_calories * 1.10) and (actual_calories >
                                                           goal_calories *
                                                           0.9):
            return 3
        # even more
...
class Ingredient(AbstractLicenseModel, models.Model):
    ...
    carbohydrates_sugar = \
        models.DecimalField(decimal_places=3,
                            ...
                            blank=True,
                            null=True,
                            verbose_name=_('Sugar content\
                                            in \
                                            carbohydrates'),
                            help_text=_('In g per \
                                       100g of product'),
                            validators=[MinValueValidator(0),
                                        MaxValueValidator(100)])
    ...
    def clean(self):
        ...
        The sum is then compared to the given
        total energy, with ENERGY_APPROXIMATION
        percent tolerance.
        '''
        ...
        if self.energy:
            energy_upper = self.energy * (1 +
                                          (self.ENERGY_APPROXIMATION /
                                           Decimal(100.0)))
            energy_lower = self.energy * (1 - (self.ENERGY_APPROXIMATION /
                                               Decimal(100.0)))

            if not ((energy_upper > energy_calculated) and (energy_calculated >
                                                            energy_lower)):
                raise ValidationError(_('Total energy is not the \
                                         approximate sum of the energy '
                                        'provided by protein, carbohydrates\
                                         and fat.'))
    ...
    def __eq__(self, other):
        ...
        logger.debug('Overwritten behaviour: comparing ingredients on \
                      values, not PK.')
        equal = True
        if isinstance(other, self.__class__):
            for i in self._meta.fields:
                if (hasattr(self, i.name) and hasattr(other, i.name) and
                   (getattr(self, i.name, None) != getattr(other,
                                                           i.name, None))):
                        equal = False
        else:
            equal = False
    ...
    def send_email(self, request):
        ...
        if self.user and self.user.email:
            translation\
                .activate(self.user.userprofile
                          .notification_language.short_name)
...
class IngredientWeightUnit(models.Model):
    ...
    amount = \
        models.DecimalField(decimal_places=2,
                            ...
                            help_text=_('Unit amount, e.g. "1 Cup" or \
                                        "1/2 spoon"'))
...
class MealItem(models.Model):
    ...
    def get_nutritional_values(self, use_metric=True):
        ...
        if self.ingredient.carbohydrates_sugar:
            nutritional_info['carbohydrates_sugar'] += self.ingredient\
                .carbohydrates_sugar \
                * item_weight / 100
