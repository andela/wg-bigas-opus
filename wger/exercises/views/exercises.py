...
def view(request, id, slug=None):
    ...
    if not backgrounds:
        ...
        for muscle in exercise.muscles.all():
            if muscle.is_front:
                backgrounds_front\
                    .append('images/muscles/main/muscle-%s.svg' % muscle.id)
            else:
                backgrounds_back\
                    .append('images/muscles/main/muscle-%s.svg' % muscle.id)
        ...
        for muscle in exercise.muscles_secondary.all():
            if muscle.is_front:
                backgrounds_front\
                    .append('images/muscles/secondary/muscle-%s.svg' %
                            muscle.id)
            else:
                backgrounds_back\
                    .append('images/muscles/secondary/muscle-%s.svg' %
        ...
        # This has to happen as the last step, so it is rendered
        # behind the muscles.
...
class ExercisesEditAddView(WgerFormMixin):
    ...
    def get_form_class(self):
        ...
        # Define the exercise form here because only at this
        ...
        class ExerciseForm(ModelForm):
            ...
            muscles = \
                ModelMultipleChoiceField(queryset=Muscle.objects.all(),
...
class ExerciseCorrectView(ExercisesEditAddView,
    ...
    messages = _('Thank you. Once the changes are reviewed the exercise \
                  will be updated.')
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        '''
        if not request\
                .user.is_authenticated() or request\
                .user.userprofile.is_temporary:
            return HttpResponseForbidden()

        return super(ExerciseCorrectView, self)\
...
class ExerciseDeleteView(WgerDeleteMixin,
    ...
    delete_message = ugettext_lazy('This will delete the exercise \
                                    from all workouts.')
...
def accept(request, pk):
    ...
    messages.success(request, _('Exercise was successfully added to \
...
def decline(request, pk):
    ...
    exercise.save()
    messages\
        .success(request, _('Exercise was successfully marked as rejected'))
    return HttpResponseRedirect(exercise.get_absolute_url())
