...
class ExerciseCategoryListView(LoginRequiredMixin,
...
class ExerciseCategoryUpdateView(WgerFormMixin,
    ...
    def get_context_data(self, **kwargs):
        context = super(ExerciseCategoryUpdateView, self)\
            .get_context_data(**kwargs)
        context['form_action'] = reverse('exercise:category:edit',
...
class ExerciseCategoryDeleteView(WgerDeleteMixin,
    ...
    # Send some additional data to the template
    def get_context_data(self, **kwargs):
        context = super(ExerciseCategoryDeleteView, self)\
            .get_context_data(**kwargs)

        context['title'] = _(u'Delete {0}?').format(self.object.name)
        context['form_action'] = reverse('exercise:category:delete',
                                         kwargs={'pk': self.object.id})
