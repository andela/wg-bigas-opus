...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        make_option('--remote-url',
                    ...
                    help='Remote URL to fetch the exercises \
                          from (default: https://wger.de)'),
    )

    help = ('Download exercise images from wger.de \
             and update the local database\n'
            '\n'
            'ATTENTION: The script will download the \
            images from the server and add them\n'
            '           to your local exercises. The \
                        exercises are identified by\n'
            '           their UUID field, if you manually \
                        edited or changed it\n'
            '           the script will not be able to match them.')

    def handle(self, **options):

        if not settings.MEDIA_ROOT:
            raise ImproperlyConfigured('Please set MEDIA_ROOT in \
                                        your settings file')
        ...
        headers = {'User-agent': default_user_agent('wger/{} + requests'
                                                    .format(get_version()))}

        # Get all exercises
        result = requests.get(exercise_api
                              .format(remote_url), headers=headers).json()
        for exercise_json in result['results']:
            ...
            self.stdout.write('')
            self.stdout.write(u"*** Processing {0} (ID: {1}, UUID: {2})"
            ...
            try:
                ...
                self.stdout.write('    Remote exercise not found in \
                                  local DB, skipping...')
                continue
            ...
            if images['count']:
                ...
                for image_json in images['results']:
                    ...
                    result = requests\
                        .get(thumbnail_api.format(remote_url, image_id),
                             headers=headers).json()

                    image_name = os.path.basename(result['original'])
                    self.stdout.write('    Fetching image {0} - {1}'
                                      .format(image_id, image_name))

                    try:
                        image = ExerciseImage.objects.get(pk=image_id)
                        self.stdout.write('    --> Image already present \
                                          locally, skipping...')
                        continue
                    except ExerciseImage.DoesNotExist:
                        self.stdout.write('    --> Image not found in local \
                                          DB, creating now...')
                        image = ExerciseImage()
                        image.pk = image_id
                ...
                self.stdout.write('    No images for this exercise, \
                                  nothing to do')
