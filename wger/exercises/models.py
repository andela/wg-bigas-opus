...
class Exercise(AbstractSubmissionModel, AbstractLicenseModel, models.Model):
    ...
    def send_email(self, request):
        ...
        if self.license_author and user.email:
            translation\
                .activate(user.userprofile.notification_language.short_name)
            url = request.build_absolute_uri(self.get_absolute_url())
            subject = _('Exercise was successfully added to the \
                        general database')
            context = {
                'exercise': self.name,
                'url': url,
    ...
    def set_author(self, request):
        ...
        if request.user.has_perm('exercises.add_exercise'):
            ...
            message = _(u'The user {0} submitted a new exercise "{1}".')\
...
class ExerciseImage(AbstractSubmissionModel,
    ...
    image = models.ImageField(verbose_name=_('Image'),
                              help_text=_('Only PNG and JPEG formats \
                                          are supported'),
                              upload_to=exercise_image_upload_dir)
    '''Uploaded image'''

    is_main = models\
        .BooleanField(verbose_name=_('Main picture'),
                      default=False,
                      help_text=_("Tick the box if you want to set \
                                   this image as the "
                                  "main one for the exercise (will \
                                   be shown e.g. in "
                                  "the search). The first image is \
                                   automatically "
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_main:
            ExerciseImage\
                .objects.filter(exercise=self.exercise).update(is_main=False)
            self.is_main = True
        else:
            if ExerciseImage\
                .objects.accepted()\
                    .filter(exercise=self.exercise).count() == 0 \
    ...
    def set_author(self, request):
        ...
        if request.user.has_perm('exercises.add_exerciseimage'):
            ...
            message = _(u'The user {0} submitted a new \
                        image "{1}" for exercise {2}.').format(
...
class ExerciseComment(models.Model):
    ...
    comment = models.CharField(max_length=200,
                               ...
                               help_text=_('A comment about how to correctly \
                                            do this exercise.'))
