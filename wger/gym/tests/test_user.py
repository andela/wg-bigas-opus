...
class GymAddUserTestCase(WorkoutManagerTestCase):
    ...
    def add_user(self, fail=False):
        ...
        response = self.client.get(reverse('gym:gym:add-user',
                                   kwargs={'gym_pk': 1}))
        ...
        response = self.client.post(reverse('gym:gym:add-user',
                                    kwargs={'gym_pk': 1}),
                                    {'first_name': 'Cletus',
                                     'last_name': 'Spuckle',
                                     'username': 'cletus',
        ...
        if fail:
            ...
            self\
                .assertEqual(len(self
                                 .client.session['gym.user']['password']), 15)
            new_user = User.objects\
                .get(pk=self.client.session['gym.user']['user_pk'])
...
class TrainerLoginTestCase(WorkoutManagerTestCase):
    ...
    def test_anonymous(self):
        ...
        '''
        response = self.client.get(reverse('core:user:trainer-login',
                                   kwargs={'user_pk': 1}))
        self.assertEqual(response.status_code, 302)
    ...
    def test_gym_trainer(self):
        ...
        permission = Permission.objects\
            .get(content_type=content_type, codename='gym_trainer')
