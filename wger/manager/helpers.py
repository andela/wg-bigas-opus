...
def render_workout_day(day, nr_of_weeks=7, images=False,
                       comments=False, only_table=False):
    '''
        ...
        for exercise in set['exercise_list']:
            ...
            if exercise['has_weight']:
                ...
                for i in exercise['setting_text'].split(u'–'):
                    setting_out.append(Paragraph
                                       (i, styleSheet["Small"], bulletText=''))
            else:
                setting_out = \
...
class WorkoutCalendar(HTMLCalendar):
    ...
    * http://uggedal.com/journal/creating-a-flexible-monthly-calendar-in-django/
    ...
    def formatday(self, day, weekday):
        ...
        # model to access the impression values directly, so they are
        # hard coded
        # here.
        if entry['session']:
            # Bad
        ...
        body\
            .append('<a href="{0}" '
                    'data-log="log-{1}" '
                    'class="btn btn-block {2} calendar-link">'
                    .format(url,
                            formatted_date,
                            background_css))
