...
class WorkoutSessionUpdateView(WgerFormMixin, LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['form_action'] = reverse('manager:session:edit',
...
class WorkoutSessionDeleteView(WgerDeleteMixin,
    ...
    def delete(self, request, *args, **kwargs):
        '''
        Delete the workout session and, if wished, \
        all associated weight logs as well
        '''
        if self.kwargs['logs'] == 'logs':
            WorkoutLog.objects\
                .filter(user=self.request.user,
                        date=self.get_object().date).delete()

        return super(WorkoutSessionDeleteView,
    ...
    def get_context_data(self, **kwargs):
        ...
        context = \
            super(WorkoutSessionDeleteView, self).get_context_data(**kwargs)
        context['form_action'] = \
            reverse('manager:session:delete', kwargs={'pk': self.object.id,
                                                      'logs': logs})
        context['title'] = _(u'Delete {0}?').format(self.object)
        if self.kwargs['logs'] == 'logs':
            context['delete_message'] = _('This will delete all weight logs \
