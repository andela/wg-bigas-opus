...
from django.utils.translation import ugettext_lazy, ugettext as _ # noqa
...
class WorkoutLogUpdateView(WgerFormMixin, UpdateView, LoginRequiredMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['form_action'] = reverse('manager:log:edit',
...
def add(request, pk):
    ...
    # We need several lists here because we need
    # to assign specific form to each
    # exercise: the entries for weight and repetitions
    # have no indicator to which
    ...
    if request.method == 'POST':
        ...
        # Make a copy of the POST data and go through it.
        # The reason for this is
        # that the form expects a value for the exercise
        # which is not present in
        # the form (for space and usability reasons)
        post_copy = request.POST.copy()

        for form_id in form_to_exercise:
            if post_copy.get('form-%s-weight' % form_id) or post_copy\
                    .get('form-%s-reps' % form_id):
                post_copy['form-%s-exercise' % form_id] = \
                    form_to_exercise[form_id].id
        ...
        if dateform\
                .is_valid() and session_form.is_valid() and formset.is_valid():
            log_date = dateform.cleaned_data['date']

            if WorkoutSession\
                    .objects.filter(user=request.user, date=log_date).exists():
                session = \
                    WorkoutSession.objects.get(user=request.user,
                                               date=log_date)
                session_form = HelperWorkoutSessionForm(data=post_copy,
                                                        instance=session)

            # Save the Workout Session only if there is not
            # already one for this date
            ...
            if not WorkoutSession\
                ...
                session = WorkoutSession\
                    .objects.get(user=request.user, date=log_date)
            ...
            return HttpResponseRedirect(reverse('manager:log:log',
                                        kwargs={'pk': day.training_id}))
    else:
        ...
        # Depending on whether there is already a workout session
        # for today, update
        # the current one or create a new one (this will be the
        # most usual case)
        if WorkoutSession.objects\
    ...
    template_data['form_action'] = \
        reverse('manager:day:log', kwargs={'pk': pk})
...
class WorkoutLogDetailView(DetailView, LoginRequiredMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        #       local instance, they could "slip" through.
                    logs = exercise_list['obj']\
                        .workoutlog_set.filter(user=self.owner_user,
                                               weight_unit__in=(1, 2),
                                               workout=self.object) \
                    ...
                    if exercise_log:
                        ...
                        workout_log[day_id][exercise_id]['log_by_date'] = \
                            entry_log
                        workout_log[day_id][exercise_id]['div_uuid'] = \
                            'div-' + str(uuid.uuid4())
                        workout_log[day_id][exercise_id]['chart_data'] = \
                            chart_data
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # Dispatch normally
        return super(WorkoutLogDetailView, self)\
...
def calendar(request, username=None, year=None, month=None):
    ...
    context['calendar'] = WorkoutCalendar(grouped_log_entries)\
    ...
    context['month_list'] = WorkoutLog.objects.filter(user=user)\
        .dates('date', 'month')
