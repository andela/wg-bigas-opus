...
class Workout(models.Model):
    ...
    def canonical_representation(self):
        ...
        if not workout_canonical_form:
            ...
            # Save to cache
            cache.set(cache_mapper.get_workout_canonical(self.pk),
                      workout_canonical_form)
    ...
    def get_current_workout(self, user):
        ...
        try:
            ...
            if schedule.schedulestep_set.count():
                ...
                active_workout = \
                    schedule.get_current_scheduled_workout().workout
            else:
                # same as above
                raise ObjectDoesNotExist
        ...
        # there are no active schedules, just return the last workout
            ...
            try:
                active_workout = \
                    Workout.objects.filter(user=user).latest('creation_date')
...
class Schedule(models.Model):
    ...
    name = models.CharField(verbose_name=_('Name'),
                            max_length=100,
                            help_text=_("Name or short description \
                                         of the schedule. "
    ...
    is_active = models\
        .BooleanField(verbose_name=_('Schedule active'),
                      default=True,
                      help_text=_("Tick the box if you want to \
                                  mark this schedule "
                                  "as your active one (will be \
                                  shown e.g. on your "
                                  "dashboard). All other schedules \
                                  will then be "
                                  "marked as inactive"))
    ...
    is_loop = models\
        .BooleanField(verbose_name=_('Is a loop'),
                      default=False,
                      help_text=_("Tick the box if you want to \
                                   repeat the schedules "
                                  "in a loop (i.e. A, B, C, A, \
                                   B, C, and so on)"))
    ...
    def get_current_scheduled_workout(self):
        ...
        while not found:
            for step in steps:
                current_limit = \
                    start_date + datetime.timedelta(weeks=step.duration)
...
class Day(models.Model):
    ...
    description = models\
        .CharField(max_length=100,
                   ...
                   help_text=_('A description of what is done on \
                                this day (e.g. '
                               '"Pull day") or what body parts \
                                are trained (e.g. '
                               '"Arms and abs")'))
    ...
    def get_canonical_representation(self):
        ...
        for set_obj in self.set_set.select_related():
            ...
            # If it's a superset, check that all exercises have
            # the same repetitions.
            # If not, just take the smallest number and drop the
            # rest, because otherwise
                ...
                for exercise in exercise_tmp:
                    if len(exercise['setting_list']) > common_reps:
                        ...
                        setting_text, setting_list, weight_list,\
                            ...
                            reps_smart_text(exercise['setting_obj_list'],
                                            set_obj)
            ...
            canonical_repr\
                .append({'obj': set_obj,
                         ...
                         'is_superset': True if len(exercise_tmp) >
                         1 else False,
...
class Set(models.Model):
    ...
    sets = models\
        .IntegerField(validators=[MinValueValidator(0),
...
class Setting(models.Model):
    ...
    Note that since adding the unit field, the name is no
    longer correct, but is
    ...
    def __str__(self):
        ...
        Return a more human-readable representation
        '''
        return u"settings for exercise {0} in set {1}"\
            .format(self.exercise.id, self.set.id)
...
class WorkoutLog(models.Model):
    ...
    Note that since adding the unit field, the name is no longer
    correct, but is
    kept for compatibility reasons (specially for the REST API).
    '''
...
class WorkoutSession(models.Model):
    ...
    notes = models.TextField(verbose_name=_('Notes'),
                             null=True,
                             blank=True,
                             help_text=_('Any notes you might want to save \
                                          about this workout '
                                         'session.'))
    '''
    ...
    impression = models.CharField(verbose_name=_('General impression'),
                                  ...
                                  help_text=_('Your impression about \
    ...
    def clean(self):
        ...
        if (not self.time_end and self.time_start) or \
            ...
            raise ValidationError(_("If you enter a time, you must enter \
                                     both start and end time."))

        if self.time_end and self.time_start and \
                self.time_start > self.time_end:
            raise ValidationError(_("The start time cannot be \
                                     after the end time."))
