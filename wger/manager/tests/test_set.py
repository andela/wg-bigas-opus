...
class SetAddTestCase(WorkoutManagerAddTestCase):
    ...
    def test_add_set(self, fail=False):
        ...
        response = self.client.post(reverse('manager:set:add',
                                    kwargs={'day_pk': 5}), post_data)
...
class SetDeleteTestCase(WorkoutManagerTestCase):
    ...
    def delete_set(self, fail=True):
        ...
        response = self.client.get(reverse('manager:set:delete',
                                   kwargs={'pk': 3}))
    ...
    def test_delete_set_other(self):
        '''
        Tests deleting a set from a workout as a logged
        user not owning the data
        '''
    ...
    def add_set(self, exercises_id):
        ...
        for exercise_id in exercises_id:
            ...
            for set_nr in range(0, nr_sets):
                post_data['exercise{0}-{1}-repetition_unit'.format(exercise_id,
                                                                   set_nr)] = 1
                post_data['exercise{0}-{1}-weight_unit'.format(exercise_id,
                                                               set_nr)] = 1
...
class SetEditEditTestCase(WorkoutManagerTestCase):
    ...
    def edit_set(self, fail=False):
        ...
        # Fetch the edit page
        response = self.client.get(reverse('manager:set:edit',
                                   kwargs={'pk': 3}))
        ...
        response = self.client.post(reverse('manager:set:edit',
                                    kwargs={'pk': 3}),
                                    {'exercise2-TOTAL_FORMS': 4,
...
class SetWorkoutCacheTestCase(WorkoutManagerTestCase):
    ...
    def test_canonical_form_cache_save(self):
        ...
        self.assertTrue(cache.get(cache_mapper
                        .get_workout_canonical(set.exerciseday.training_id)))
...
class SettingWorkoutCacheTestCase(WorkoutManagerTestCase):
    ...
    def test_canonical_form_cache_save(self):
        ...
        self.assertTrue(cache.get(cache_mapper
                                  .get_workout_canonical(workout_id)))
