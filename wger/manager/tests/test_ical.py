...
class WorkoutICalExportTestCase(WorkoutManagerTestCase):
    ...
    def export_ical_token(self):
        '''
        Helper function that checks exporing an ical
        file using tokens for access
        '''

        user = User.objects.get(username='test')
        uid, token = make_token(user)
        response = self.client.get(reverse('manager:workout:ical',
                                   kwargs={'pk': 3,
                                           'uidb64': uid,
                                           'token': token}))
    ...
    def test_export_ical_other(self):
        '''
        Tests exporting a workout as an ical file as a
...
class ScheduleICalExportTestCase(WorkoutManagerTestCase):
    ...
    def export_ical_token(self):
        ...
        user = User.objects.get(username='test')
        uid, token = make_token(user)
        response = self.client.get(reverse('manager:schedule:ical',
                                   kwargs={'pk': 2,
                                           'uidb64': uid,
                                           'token': token}))
    ...
    def test_export_ical_other(self):
        '''
        Tests exporting a schedule as an ical file
        as a logged user not owning the data
        '''
