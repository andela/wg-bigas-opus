...
class WorkoutPdfLogExportTestCase(WorkoutManagerTestCase):
    ...
    def export_pdf_token(self):
        ...
        response = self.client.get(reverse('manager:workout:pdf-log',
                                   kwargs={'id': 3,
                                           'uidb64': uid,
                                           'token': token}))
    ...
    def export_pdf_token_wrong(self):
        '''
        Helper function to test exporting a workout
        as a pdf using a wrong token
        '''
...
class WorkoutPdfTableExportTestCase(WorkoutManagerTestCase):
    ...
    def export_pdf_token(self):
        ...
        user = User.objects.get(username='test')
        uid, token = make_token(user)
        response = self.client.get(reverse('manager:workout:pdf-table',
                                   kwargs={'id': 3,
                                           'uidb64': uid,
                                           'token': token}))
