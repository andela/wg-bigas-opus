...
class WorkoutTimerTestCase(WorkoutManagerTestCase):
    ...
    def test_timer_no_weight(self):
        ...
        response = self.client.get(reverse('manager:workout:timer',
                                   kwargs={'day_pk': 5}))
    ...
    def timer(self, fail=True, pause_active=True, pause_seconds=90):
        ...
        if fail:
            ...
            current_step = step_list.pop()
            self.assertEqual(current_step['exercise'], Exercise.objects
                                                               .get(pk=2))
            self.assertEqual(current_step['reps'], 10)
            self.assertEqual(current_step['step_nr'],
                             list_length - len(step_list))
            self.assertEqual(math.floor(current_step['step_percent']),
                             math.floor(current_step['step_nr'] *
                             (100.0 / list_length)))
            ...
            current_step = step_list.pop()
            self.assertEqual(current_step['exercise'],
                             Exercise.objects.get(pk=2))
            self.assertEqual(current_step['reps'], 10)
            self.assertEqual(current_step['step_nr'],
                             list_length - len(step_list))
            self.assertEqual(math.floor(current_step['step_percent']),
