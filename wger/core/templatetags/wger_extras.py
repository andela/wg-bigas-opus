...
def get_current_settings(exercise, set_id):
    ...
    We need to do this here because it's not possible to pass arguments
    to function in
    the template, and we are only interested on the settings that belong
    to the current
    set
    '''
...
def pagination(paginator, page):
    ...
    # For very long lists (e.g. the English ingredient
    # with more than 8000 items)
    # we muck around here to remove the pages not inmediately
    # 'around' the current
    ...
    if paginator.num_pages > PAGINATION_MAX_TOTAL_PAGES:
        ...
        for i in range(page.number -
                       PAGINATION_PAGES_AROUND_CURRENT, page.number + 1):
            if i > 0:
                start_page = i
                break
...
def auto_link_css(flavour='full', css=''):
    '''
    Adds the appropriate classes to a sidebar link
    depending on the site version
...
class SpacelessNode(template.base.Node):
    ...
    def render(self, context):
        if settings.WGER_SETTINGS['REMOVE_WHITESPACE']:
            ...
            return strip_spaces_between_tags(self.nodelist.render(context)
                                             .strip())
        else:
...
def is_checkbox(field):
    '''
    Tests if a field element is a checkbox, as it needs
...
def is_fileupload(field):
    '''
    Tests if a field element is a file upload, as
    it needs to be handled slightly different
