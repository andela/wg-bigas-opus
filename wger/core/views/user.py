...
def delete(request, user_pk=None):
    '''
    Delete a user account and all his data, requires password
    confirmation first
    ...
    if user_pk:
        ...
        if not request.user.has_perm('gym.manage_gyms') \
                and (not request.user.has_perm('gym.manage_gym')
                     or request
                     .user.userprofile.gym_id != user.userprofile.gym_id
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request,
                             _('Account "{0}" was successfully \
                                deleted').format(user.username))
...
def registration(request):
    ...
    if request.user.is_authenticated() and not request \
            .user.userprofile.is_temporary:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            language = Language.objects \
                .get(short_name=translation.get_language())
...
class UserDeactivateView(LoginRequiredMixin,
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if (request
            .user.has_perm('gym.manage_gym') or request
            .user.has_perm('gym.gym_trainer')) \
            and edit_user \
                .userprofile.gym_id != request.user.userprofile.gym_id:
            return HttpResponseForbidden()

        return super(UserDeactivateView, self) \
    ...
    def get_redirect_url(self, pk):
        ...
        edit_user.save()
        messages.success(self.request, _('The user was successfully \
                                         deactivated'))
...
class UserActivateView(LoginRequiredMixin,
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if (request
            .user.has_perm('gym.manage_gym') or request
            .user.has_perm('gym.gym_trainer')) \
            and edit_user.userprofile.gym_id != request \
    ...
    def get_redirect_url(self, pk):
        ...
        edit_user.save()
        messages.success(self.request, _('The user was \
...
class UserDetailView(LoginRequiredMixin,
    ...
    def get_context_data(self, **kwargs):
        ...
        context['weight_entries'] = WeightEntry\
            .objects.filter(user=self.object)\
            .order_by('-date')[:5]
        context['nutrition_plans'] = NutritionPlan\
            .objects.filter(user=self.object)\
            .order_by('-creation_date')[:5]
        context['session'] = WorkoutSession\
            .objects.filter(user=self.object).order_by('-date')[:10]
        context['admin_notes'] = AdminUserNote\
            .objects.filter(member=self.object)[:5]
...
class UserListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        for u in User.objects\
                .select_related('usercache', 'userprofile__gym').all():
