...
def demo_entries(request):
    ...
    if (((not request.user
        .is_authenticated() or request.user.userprofile.is_temporary)
        ...
        messages.success(request, _('We have created sample workout, \
...
def dashboard(request):
    ...
    try:
        plan = NutritionPlan.objects \
            .filter(user=request.user).latest('creation_date')
...
class FeedbackClass(FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['extend_template'] = 'base_empty.html' if self.request \
            .is_ajax() else 'base.html'
        return context
    ...
    def get_form_class(self):
        ...
        '''
        if self.request.user.is_anonymous() or self \
    ...
    def form_valid(self, form):
        ...
        '''
        messages.success(self.request, _('Your feedback was successfully sent.\
                                         Thank you!'))
