...
class Command(BaseCommand):
    ...
    help = 'Clears the application cache. \
            You *must* pass an option selecting ' \
    ...
    def handle(self, **options):
        ...
        if (not options['clear_template']
            ...
            raise CommandError('Please select what cache \
                                you need to delete, see help')
                        ...
                        for day in WorkoutLog\
                            .objects.filter(user=user,
                                    ...
                                    .dates('date',
                                           'day'):
                            if int(options['verbosity']) >= 3:
                                self.stdout.write("      Day {0}"
                                                  .format(day.day))
                            reset_workout_log(user.id, entry.year,
                                              entry.month, day)
