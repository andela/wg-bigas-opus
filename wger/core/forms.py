...
class UserEmailForm(forms.ModelForm):
    email = EmailField(label=_("Email"),
                       help_text=_("Used for password resets and, optionally, \
                                    email reminders."),
                       required=False)
    ...
    def clean_email(self):
        ...
        However, this check should only
        be performed when the user changes his
...
class RegistrationForm(UserCreationForm, UserEmailForm):
    ...
    captcha = ReCaptchaField(attrs={'theme': 'clean', 'lang': 'en'},
                             ...
                             help_text=_('As a security measure, please enter \
                                         the previous words'))
...
class FeedbackRegisteredForm(forms.Form):
    ...
    contact = forms.CharField(max_length=50,
                              min_length=10,
                              label=_('Contact'),
                              help_text=_('Some way of answering \
                                           you (email, etc.)'),
                              required=False)
