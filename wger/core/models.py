...
class UserProfile(models.Model):
    ...
    show_comments = models\
        .BooleanField(verbose_name=_('Show exercise comments'),
...
7000 entries, but can be somewhat overwhelming and make the \
search difficult.'''),
        default=True)

    workout_reminder_active = models\
        .BooleanField(verbose_name=_('Activate workout reminders'),
    ...
    workout_reminder = IntegerField(verbose_name=_('Remind before expiration'),
                                    help_text=_('The number of days you want \
                                                to be reminded '
                                                'before a workout expires.'),
                                    default=14,
                                    validators=[MinValueValidator(1),
                                                MaxValueValidator(30)])
    workout_duration = IntegerField(verbose_name=_('Default duration \
                                                    of workouts'),
                                    help_text=_('Default duration in \
                                                weeks of workouts not '
                                                'in a schedule. Used \
                                                for email workout '
                                                'reminders.'),
                                    default=12,
    ...
    timer_active = models\
        .BooleanField(verbose_name=_('Use pauses in workout timer'),
    ...
    timer_pause = IntegerField(verbose_name=_('Default duration of \
                                              workout pauses'),
                               help_text=_('Default duration in seconds \
                                            of pauses used by '
    ...
    calories = IntegerField(verbose_name=_('Total daily calories'),
                            help_text=_('Total caloric intake, \
                                        including e.g. any surplus'),
                            default=2500,
                            blank=False,
                            null=True,
    ...
    ro_access = models\
        .BooleanField(verbose_name=_('Allow external access'),
                      help_text=_('Allow external users to access\
                                   your workouts and '
                                  'logs in a read-only mode. \
                                   You need to set this '
                                  'before you can share links e.g. \
                                   to social media.'),
                      default=False)
    '''Allow anonymous read-only access'''

    num_days_weight_reminder = models\
        .IntegerField(verbose_name=_('Automatic reminders for weight '
                                     'entries'),
    ...
    def clean(self):
        ...
        if ((self.sleep_hours and self.freetime_hours and self.work_hours)
           and (self.sleep_hours + self.freetime_hours +
                self.work_hours) > 24):
    ...
    def calculate_bmi(self):
        ...
        weight = self\
            .weight if self.use_metric else AbstractWeight(self
                                                           .weight, 'lb').kg
    ...
    def calculate_basal_metabolic_rate(self, formula=1):
        ...
        weight = self.weight if self.use_metric else AbstractWeight(self
                                                                    .weight,
                                                                    'lb').kg
...
class DaysOfWeek(models.Model):
    ...
    This model is needed so that 'Day' can have multiple days of
    the week selected
    '''
...
class License(models.Model):
    ...
    full_name = models\
        .CharField(max_length=60,
                   ...
                   help_text=_('If a license has been localized, e.g. \
                                the Creative '
                               'Commons licenses for the different \
                                countries, add '
    ...
    url = models.URLField(verbose_name=_('Link'),
                          help_text=_('Link to license text \
