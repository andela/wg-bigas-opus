...
parser = argparse.ArgumentParser(description='Data generator. Please \
                                 consult the documentation')
subparsers = parser.add_subparsers(help='The kind of entries \
...
session_parser.add_argument('impression_sessions',
                            action='store',
                            help='Impression for the sessions, \
                                  default: random',
                            default='random',
...
weight_parser.add_argument('--base-weight',
                           action='store',
                           help='Default weight for the \
                                 entry generation, default = 80',
                           type=int,
                           default=80)

# Nutrition options
nutrition_parser = subparsers.add_parser('nutrition',
...
if hasattr(args, 'number_users'):
    ...
    with open(os.path
              .join('csv', 'first_names_{0}.csv'
                    .format(args.country))) as name_file:
        name_reader = csv.reader(name_file)
        for row in name_reader:
            first_names.append(row)

    with open(os.path.join('csv', 'last_names_{0}.csv'
                           .format(args.country))) as name_file:
        name_reader = csv.reader(name_file)
        for row in name_reader:
        ...
        for i in range(1, args.number_workouts):
            ...
            for day in day_list[0:nr_of_days]:
                ...
                for exercise in day_exercises:
                    ...
                    setting = Setting(set=day_set, exercise=exercise,
        ...
        for date in WorkoutLog.objects.filter(user=user).dates('date', 'day'):
            ...
            if not WorkoutSession.objects.filter(user=user,
                ...
                if args.impression_sessions == 'good':
                    ...
                    session.impression = random \
                        .choice([WorkoutSession.IMPRESSION_GOOD,
...
if hasattr(args, 'number_weight'):
    print("** Generating {0} weight entries per user"
          .format(args.number_weight))
    ...
    for user in userlist:
        ...
        existing_entries = [i.date for i in WeightEntry.objects
                            .filter(user=user)]
...
if hasattr(args, 'number_nutrition_plans'):
    print("** Generating {0} nutrition plan(s) per user"
          .format(args.number_nutrition_plans))
